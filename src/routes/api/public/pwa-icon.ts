        ...
        try {
          ...
          if (url.searchParams.get("audit") === "cashback") {
            try {
              ...
              // 2. Busca todas as entradas de cashback_entries com select("*")
              ...
              // Também busca CashbackMovimentacao caso exista para comparação
              const { data: movimentacoes = [], error: movErr } = await supabaseAdmin
                .from("CashbackMovimentacao" as any)
                .select("*")
                .limit(5000);

              // 3. Busca todas as vendas canceladas/estornadas
              const { data: cancelledSales = [], error: salesErr } = await supabaseAdmin
                .from("sales")
              ...
              // Análise 1: Reconciliação por cliente
              ...
              // Análise 5: Rastrear saldo negativo histórico
              const negativeBalanceIncidents: any[] = [];
              const runningBalances = new Map<string, number>();

              // Ordena entries por created_at se existir
              ...
              // Análise 2: Crédito indevido em vendas canceladas
              ...
              // Análise 3: Vínculo com cliente nulo ou inexistente
              ...
              // Análise 4: Duplicidade de crédito na mesma sale_id
              ...
              return new Response(JSON.stringify({
                ...
                total_movimentacoes_count: movimentacoes?.length ?? 0,
            ...
            } catch (auditCatchErr: any) {
              return new Response(JSON.stringify({
                success: false,
                catch_error: auditCatchErr.message,
                stack: auditCatchErr.stack
              }, null, 2), {
                status: 200,
                headers: { "Content-Type": "application/json" }
              });
