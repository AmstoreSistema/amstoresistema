...
import { HandCoins, Wallet, Search, User, ChevronRight, AlertCircle, Clock } from "lucide-react";
...
function CreditPage() {
    ...
    const filteredClients = useMemo(() => {
        return clientStats.filter(c => {
            ...
            if (filter === 'vencidos')
                return matchesTerm && c.isOverdue;
            if (filter === 'em_dia')
                return matchesTerm && !c.isOverdue;
            return matchesTerm;
        });
    ...
    return (<div className="space-y-6 animate-in fade-in duration-500">
      <PageHeader title="Gestão de Fiado" description="Controle de vendas a prazo por cliente" icon={HandCoins}/>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar por cliente..." className="pl-10 h-11 rounded-2xl bg-card border-border/40" value={term} onChange={e => setTerm(e.target.value)}/>
        </div>
        <div className="flex bg-muted/30 p-1 rounded-2xl gap-1">
          {['todos', 'vencidos', 'em_dia'].map((f) => (<Button key={f} variant="ghost" size="sm" className={cn("rounded-xl px-4 font-bold capitalize transition-all", filter === f ? "bg-card text-gold shadow-sm" : "text-muted-foreground")} onClick={() => setFilter(f)}>
              {f.replace('_', ' ')}
            </Button>))}
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {salesLoading ? ([1, 2, 3, 4, 5, 6].map(i => <div key={i} className="h-40 bg-card animate-pulse rounded-[2rem]"/>)) : filteredClients.length === 0 ? (<Card className="col-span-full rounded-[2rem] border-dashed bg-muted/20 border-border/40">
              <CardContent className="p-12 text-center text-muted-foreground">Nenhum cliente encontrado com os filtros atuais.</CardContent>
           </Card>) : (filteredClients.map(c => (<Card key={c.clientId} className="group overflow-hidden rounded-[2rem] border-border/40 bg-card hover:bg-muted/10 transition-all shadow-sm hover:shadow-md border-t-4 border-t-gold">
              <CardContent className="p-6">
                 <div className="flex justify-between items-start mb-4">
                    <div className="size-12 rounded-2xl bg-muted/50 flex items-center justify-center shrink-0">
                       <User className="size-6 text-muted-foreground"/>
                    </div>
                    {c.isOverdue && (<Badge className="bg-destructive/10 text-destructive border-none font-black flex gap-1 items-center">
                 ...
                 <div className="space-y-4">
                    ...
                    <Button className="w-full rounded-xl font-bold gap-2 text-gold bg-gold/5 hover:bg-gold/10" variant="ghost" onClick={() => {
      ...
      <ClientDetailsModal isOpen={clientDetailsOpen} onClose={() => setClientDetailsOpen(false)} client={selectedClient}/>

      <SaleDetailsModal saleId={selectedSaleId} isOpen={saleDetailsOpen} onClose={() => setSaleDetailsOpen(false)}/>
    </div>);
}
