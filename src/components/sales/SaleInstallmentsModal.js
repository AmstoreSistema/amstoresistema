...
import { Calendar, Banknote, Plus, Trash2, Save, Edit, AlertCircle } from "lucide-react";
...
export function SaleInstallmentsModal({ open, onOpenChange, saleId }) {
    ...
    React.useEffect(() => {
        if (isEditing && installments.length > 0) {
            setEditList(installments.map((i) => ({
                ...i,
    ...
    const handlePartialPay = async () => {
        ...
        if (!inst || !amount || Number(amount) <= 0)
            return;
        try {
    ...
    const remainingTotal = unpaidInstallments.reduce((acc, curr) => acc + Number(curr.remaining_amount ?? curr.amount), 0);
    return (<>
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-2xl p-0 overflow-hidden rounded-[2rem] border-none shadow-2xl">
          <DialogHeader className="px-8 py-5 border-b border-border/40 bg-card/50 backdrop-blur-xl flex-row items-center justify-between">
            ...
            {!isEditing && (<Button variant="ghost" size="sm" className="rounded-xl gap-2 font-bold" onClick={() => setIsEditing(true)}>
                <Edit className="size-4"/> Ajustar Parcelas
              </Button>)}
          </DialogHeader>

          <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
            {isLoading ? (<div className="py-10 text-center text-muted-foreground animate-pulse">Carregando parcelas...</div>) : isEditing ? (<div className="space-y-4">
                ...
                {editList.map((inst, idx) => (<div key={idx} className="flex gap-3 items-end p-4 rounded-2xl border border-border/40 bg-card/50">
                    <div className="w-12">
                    ...
                    <div className="flex-1">
                      ...
                      <Input type="date" value={inst.due_date} onChange={e => {
                    ...
                    <Button variant="ghost" size="icon" className="h-10 w-10 text-destructive rounded-xl" onClick={() => setEditList(editList.filter((_, i) => i !== idx))}>
                      <Trash2 className="size-4"/>
                    </Button>
                  </div>))}
                
                <Button variant="outline" className="w-full rounded-2xl border-dashed border-2 gap-2 h-12" onClick={() => setEditList([...editList, {
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <Button className="flex-1 rounded-2xl bg-gold text-black font-bold hover:bg-gold/90 gap-2" onClick={handleSaveEdits}>
                    <Save className="size-4"/> Salvar Alterações
                  </Button>
                </div>
              </div>) : installments.length === 0 ? (<div className="py-10 text-center text-muted-foreground">Nenhuma parcela encontrada.</div>) : (installments.map((inst) => {
            const isOverdue = inst.status !== 'paid' && new Date(inst.due_date) < new Date();
            return (<div key={inst.id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${isOverdue ? 'border-destructive/30 bg-destructive/5' : 'border-border/40 bg-card hover:bg-muted/5'}`}>
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-black text-sm">{inst.installment_number}ª Parcela</span>
                        {inst.status === 'paid' ? (<Badge className="bg-success/10 text-success border-none text-[9px] h-4">Pago</Badge>) : inst.status === 'partial' ? (<Badge className="bg-warning/10 text-warning border-none text-[9px] h-4">Parcial</Badge>) : isOverdue ? (<Badge className="bg-destructive text-white border-none text-[9px] h-4 animate-pulse">Atrasado</Badge>) : (<Badge className="bg-muted/20 text-muted-foreground border-none text-[9px] h-4">Pendente</Badge>)}
                      </div>
                      <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-tighter">
                        Vencimento: {new Date(inst.due_date).toLocaleDateString('pt-BR')}
                      </p>
                      {Number(inst.paid_amount) > 0 && (<p className="text-[9px] text-success font-bold uppercase">
                          Pago: {brl(inst.paid_amount)} | Restante: {brl(inst.remaining_amount)}
                        </p>)}
                    </div>
                    ...
                    <div className="text-right flex items-center gap-4">
                      <div className="flex flex-col items-end">
                        ...
                        {inst.status !== 'paid' && inst.remaining_amount !== inst.amount && (<span className="text-[9px] text-muted-foreground line-through opacity-50">{brl(inst.amount)}</span>)}
                      </div>

                      {inst.status !== 'paid' && (<div className="flex gap-2">
                          <Button size="sm" variant="outline" className="rounded-xl h-8 text-[9px] font-black border-success/30 text-success hover:bg-success/10" onClick={() => setPayModal({ open: true, inst, amount: String(inst.remaining_amount ?? inst.amount) })}>
                            PAG. PARCIAL
                          </Button>
                          <Button size="sm" className="rounded-xl h-8 text-[10px] font-black bg-success hover:bg-success/90" onClick={() => {
                        setPayModal({
                            open: true,
                            inst,
          ...
          <div className="p-6 border-t border-border/40 bg-muted/5 flex gap-3">
             <Button className="flex-1 rounded-xl bg-gold text-black font-black hover:bg-gold/90 gap-2" onClick={() => setSecretaryOpen(true)} disabled={unpaidInstallments.length === 0}>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input type="number" value={payModal.amount} onChange={e => setPayModal({ ...payModal, amount: e.target.value })} className="pl-10 h-12 rounded-2xl text-lg font-black text-success border-success/20 bg-success/5"/>
              </div>
            ...
            <Button className="w-full h-12 rounded-2xl bg-success text-white font-black hover:bg-success/90 shadow-lg shadow-success/20" onClick={handlePartialPay}>
              CONFIRMAR PAGAMENTO
            </Button>
      ...
      <PaymentSecretaryModal open={secretaryOpen} onOpenChange={setSecretaryOpen} saleId={saleId} clientName={sale?.client_name || "Cliente"} totalAmount={Number(sale?.total_amount || 0)} remainingAmount={remainingTotal} installments={installments}/>
    </>);
}
