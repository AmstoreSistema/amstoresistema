...
import { ShoppingBag, Printer, History } from "lucide-react";
import { useServerFn } from "@tanstack/react-start";
import { getSaleDetails } from "@/lib/sales.functions";
import { useQuery } from "@tanstack/react-query";
import { Button } from "@/components/ui/button";
import { ReceiptModal } from "./ReceiptModal";
export function SaleDetailsModal({ saleId, isOpen, onClose, open, onOpenChange }) {
    ...
    return (<>
        ...
        <DialogContent className="max-w-4xl p-0 overflow-hidden bg-[#F8F9FB] border-none shadow-2xl flex flex-col h-[90vh] sm:rounded-[2rem]">
          <div className="flex items-center justify-between p-6 bg-white border-b relative shrink-0">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {sale && (<div className="flex gap-2">
                      <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded uppercase", sale.status === 'paid' || sale.status === 'completed' || sale.status === 'finalizado' ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700")}>
            ...
            <div className="flex items-center gap-2 mr-8">
              <Button variant="outline" size="sm" className="rounded-xl font-bold h-9 gap-2 shadow-sm" onClick={() => setReceiptOpen(true)}>
              ...
              <div className="p-6 space-y-8">
                ...
                <div>
                  ...
                  <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
                    {isLoading ? (<div className="py-8 text-center text-muted-foreground">Carregando itens...</div>) : items.length === 0 ? (<div className="py-8 text-center text-muted-foreground">Nenhum item encontrado.</div>) : (<div className="divide-y divide-gray-50">
                        {items.map((item, i) => (<div key={i} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                            <div className="flex-1">
                              <div className="font-bold text-foreground text-sm uppercase">{item.products?.name || "Produto"}</div>
                              {item.numeracao && (<div className="text-[10px] font-bold text-blue-600 mt-0.5">Nº {item.numeracao}</div>)}
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {sale?.payment_method === 'Fiado' && (<div className="text-right">
                  ...
                  <div className="space-y-4">
                    ...
                    {installments.length > 0 && (<div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-3 block">Plano de Parcelamento</p>
                        <div className="space-y-2">
                          {installments.map((inst, i) => (<div key={i} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                              <div className="flex items-center gap-2">
                                <div className={cn("size-6 rounded-full flex items-center justify-center text-[10px] font-bold", inst.status === 'paid' || inst.status === 'pago' ? "bg-green-100 text-green-700" :
                    ...
                    <div className="space-y-2">
                      ...
                      {isLoading ? (<div className="py-4 text-center text-muted-foreground">Carregando...</div>) : payments.length === 0 ? (<div className="py-6 text-center text-muted-foreground bg-white rounded-2xl border border-dashed text-xs">
                          Nenhum pagamento registrado.
                        </div>) : payments.map((pay, i) => (<div key={i} className="bg-white rounded-xl p-4 flex items-center justify-between border border-gray-100 shadow-sm">
                          <div>
      ...
      {sale && (<ReceiptModal open={receiptOpen} onOpenChange={setReceiptOpen} sale={{
                ...sale,
                items: items.map((it) => ({
                    ...it,
