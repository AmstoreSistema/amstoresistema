-- 1. Garantir que a coluna numeracao existe na tabela sale_items
DO $$
BEGIN
...
-- 2. Atualizar a função RPC create_complete_sale
...
BEGIN
    ...
    LOOP
        ...
        -- Adjust stock
        -- 1. Sempre baixa do estoque total no registro original (stock_products se tiver stock_id)
        IF v_stock_id IS NOT NULL THEN
            UPDATE public.stock_products
            SET 
                quantidade_disponivel = GREATEST(0, quantidade_disponivel - v_qty),
                -- 2. Se tiver numeração, baixa da grade JSONB 'numeracoes'
                numeracoes = CASE 
                    WHEN v_numeracao IS NOT NULL AND v_numeracao <> '' AND numeracoes ? v_numeracao
                    THEN jsonb_set(
                        numeracoes, 
                        ARRAY[v_numeracao], 
                        to_jsonb(GREATEST(0, (COALESCE((numeracoes->>(v_numeracao))::NUMERIC, 0) - v_qty)))
                    )
                    ELSE numeracoes
                END
            WHERE id = v_stock_id;
        ...
        -- 3. Baixa do estoque total no registro principal de produtos (products)
    ...
    IF NOT p_is_debt AND p_paid_amount > 0 THEN
        INSERT INTO public.transactions (
            ...
            p_paid_amount, 'income', 'Venda #' || COALESCE(p_sale_code, substring(v_sale_id::text from 1 for 8)), v_sale_id, 'Venda', p_financial_account_id, 'pago', p_created_at
    ...
    IF p_client_id IS NOT NULL THEN
        ...
        IF p_cashback_used > 0 THEN
            ...
            VALUES (p_client_id, p_cashback_used, 'used', 'Resgate em venda #' || COALESCE(p_sale_code, substring(v_sale_id::text from 1 for 8)));
        END IF;
        ...
        IF p_cashback_earned > 0 THEN
            ...
            INSERT INTO public.cashback_entries (client_id, amount, kind, description)
            VALUES (p_client_id, p_cashback_earned, 'earned', 'Ganho em venda #' || COALESCE(p_sale_code, substring(v_sale_id::text from 1 for 8)));
        END IF;
    END IF;
